...
このリポジトリの全てのサンプルアプリは、 [Sora iOS SDK 2026.2.1](https://github.com/shiguredo/sora-ios-sdk/releases/tag/2026.2.1) を使用しています。
...
配信時の接続設定で、カメラを有効にして接続することで配信者の映像も同時に配信することができます。
このとき、画面とカメラの配信は個別の接続で配信します。

> [!attention]
> ScreenCastSample では配信者の映像をサムネイルとして画面に表示し、それを画面キャプチャーして配信するため、
> ゲーム画面と配信者映像が1つのストリームで配信されているように見えます。
> しかし実際のゲーム画面と配信者の映像は個別の接続による配信のため、受信側クライアントでの配置調整が必要です。
...
DataChannel 経由のシグナリング利用時に JSON-RPC 2.0 over DataChannel で一部の HTTP API をクライアントから呼び出すことができる機能の動作を確認できるサンプルです。

リクエスト/レスポンスの内容は画面の履歴テーブルに表示されます。
