...
private let logger = SamplesLogger.tagged("DecoStreamingSDKManager")
...
class DecoStreamingSoraSDKManager {
  /// DecoStreamingSoraSDKManagerのシングルトンインスタンスです。
  static let shared = DecoStreamingSoraSDKManager()
  ...
  func connect(
    ...
    // 必須となる設定は URL、チャネル ID、ロールです。
    // その他の設定にはデフォルト値が指定されていますが、ここで必要に応じて自由に調整することが可能です。
    var configuration = Configuration(
      urlCandidates: DecoStreamingEnvironment.urls,
      channelId: channelId,
      role: role)
    ...
    configuration.signalingConnectMetadata = DecoStreamingEnvironment.signalingConnectMetadata
