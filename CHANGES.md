...
- [UPDATE] Swift 6 言語モードでビルドできるようにする
  ...
  - `@preconcurrency import Sora` で並行性チェックを緩和して import する
  - SDK のコールバッククロージャーを `@Sendable` 化してアクター隔離を外し、`Task { @MainActor in }` で束ねる (Xcode 26 の実行時隔離チェックによる EXC_BREAKPOINT 対策)
