...
class ScreenRecorder {
    let frameRendererQueue: DispatchQueue = DispatchQueue(label: "ScreenCastSample.ScreenRenderer.frameRendererQueue", qos: .userInteractive)
    var frameRendererSemaphore: DispatchSemaphore = DispatchSemaphore(value: 1)
    ...
    private func onDisplayLink(_ displayLink: CADisplayLink) {
        ...
        frameRendererQueue.async {
            ...
            guard let pixelBufferPool = self.outputBufferPool,
                let colorSpace = self.outputColorSpace else {
                    return
            }
            ...
            guard let bitmapContext = CGContext.init(data: CVPixelBufferGetBaseAddress(pixelBuffer),
            ...
            bitmapContext.concatenate(CGAffineTransform(a: 1, b: 0, c: 0, d: -1, tx: 0, ty: self.inputViewSize.height)) // 縦方向反転用のTransformを適用する。CGContextの座標系は左下基準なので、上下反転させる必要がある。
            ...
            DispatchQueue.main.sync {
                ...
                for  window in UIApplication.shared.windows {
                    window.drawHierarchy(in: CGRect(origin: .zero, size: self.inputViewSize), afterScreenUpdates: false)
                }
            }
